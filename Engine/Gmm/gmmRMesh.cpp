    ...
    int nPolygon, int nPolyVertex, int nMeshVertex)
{
    ...
    case FbxLayerElement::eByPolygon:
        ...
        break;
    default:
        OSG_WARN << "getVertexIndex: unsupported GMM mapping mode" << std::endl;
    }
    ...
    const char* szName)
{
    ...
    if (geomType & GEOMETRY_MORPH)
    {
        for (unsigned i = 0; i < pGeode->getNumDrawables(); ++i)
        {
            ...
            osgAnimation::UpdateMorph* morphupdate = new osgAnimation::UpdateMorph(morph.getName());
            morphupdate->addTarget(morph.getName());// add by LiuTao,2025.01.26
            pGeode->addUpdateCallback(morphupdate);
            ...
            for (int nBlendShape = 0; nBlendShape < nDeformerBlendShapeCount; ++nBlendShape)
            {
                ...
                for (int nBlendShapeChannel = 0; nBlendShapeChannel < nBlendShapeChannelCount; ++nBlendShapeChannel)
                {
                    ...
                    {
                        OSG_WARN << "Multiple GMM Target Shapes, only the first will be used" << std::endl;
                    }
                    ...
                    readAnimation(pNode, gmmScene, morph.getName(), pAnimationManager, fbxMesh,
        ...
        for (int nBlendShape = 0; nBlendShape < nDeformerBlendShapeCount; ++nBlendShape)
        {
            ...
            for (int nBlendShapeChannel = 0; nBlendShapeChannel < nBlendShapeChannelCount; ++nBlendShapeChannel)
            {
                ...
                for (int gmmIndex = 0; gmmIndex < nControlPoints; ++gmmIndex)
                {
                    osg::Vec3d vPos = convertVec3(pControlPoints[gmmIndex]);
                    for (FbxToOsgVertexMap::const_iterator it =
                        fbxToOsgVertMap.find(gmmIndex);
    ...
    FbxAMatrix gmmGeometricTransform;
    gmmGeometricTransform.SetTRS(
        pNode->GeometricTranslation.Get(),
        pNode->GeometricRotation.Get(),
        pNode->GeometricScaling.Get());
    const double* pGeometricMat = gmmGeometricTransform;
    ...
    if (geomType & GEOMETRY_RIG)
    {
        ...
        {
            FbxAMatrix gmmTransformMatrix;
            pSkin->GetCluster(0)->GetTransformMatrix(gmmTransformMatrix);
            const double* pTransformMatrix = gmmTransformMatrix;

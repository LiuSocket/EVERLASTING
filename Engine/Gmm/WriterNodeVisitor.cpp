...
/*
 * GMM writer for Open Scene Graph
 *
 * Copyright (C) 2009
 *
    ...
    _osgImage(NULL)
{
    ...
    if (mat)
    {
        ...
        if (attribute)
        {
            ...
            {
                OSG_WARN << "GMM Writer: Reversed face (culled FRONT) not supported yet." << std::endl;
            }
            else if (mode != osg::CullFace::BACK)
            {
                assert(mode == osg::CullFace::FRONT_AND_BACK);
                OSG_WARN << "GMM Writer: Invisible face (culled FRONT_AND_BACK) not supported yet." << std::endl;
            }
        }

        _gmmMaterial = FbxSurfacePhong::Create(pSdkManager, mat->getName().c_str());
        if (_gmmMaterial)
        {
            _gmmMaterial->DiffuseFactor.Set(1);
            _gmmMaterial->Diffuse.Set(FbxDouble3(
                diffuse.x(),
                diffuse.y(),
                diffuse.z()));

            _gmmMaterial->TransparencyFactor.Set(transparency);

            _gmmMaterial->Ambient.Set(FbxDouble3(
                ambient.x(),
                ambient.y(),
                ambient.z()));

            _gmmMaterial->Emissive.Set(FbxDouble3(
                emission.x(),
                emission.y(),
                emission.z()));

            _gmmMaterial->Specular.Set(FbxDouble3(
                specular.x(),
                specular.y(),
                specular.z()));

            _gmmMaterial->Shininess.Set(shininess);
        }
    }
    if (tex && tex->getImage(0))
    {
        ...
        _gmmTexture = FbxFileTexture::Create(pSdkManager, relativePath.c_str());
        _gmmTexture->SetFileName(relativePath.c_str());
        // Create a GMM material if needed
        if (!_gmmMaterial)
        {
            _gmmMaterial = FbxSurfacePhong::Create(pSdkManager, relativePath.c_str());
        }
        ...
        if (_gmmMaterial)
        {
            _gmmMaterial->Diffuse.ConnectSrcObject(_gmmTexture);
        }
    }
}
                                                  ...
                                                  FbxMesh*         mesh)
{
    ...
    // For now, GMM writer only supports normals bound per vertices
...
void WriterNodeVisitor::apply(osg::Group& node)
{
    if (_firstNodeProcessed)
    {
        ...
        FbxNode* nodeGMM = FbxNode::Create(_pSdkManager, node.getName().empty() ? "DefaultName" : node.getName().c_str());
        _curFbxNode->AddChild(nodeGMM);
        _curFbxNode = nodeGMM;

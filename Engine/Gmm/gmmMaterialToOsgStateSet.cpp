...
{
    FbxMaterialMap::const_iterator it = _gmmMaterialMap.find(pFbxMat);
    if (it != _gmmMaterialMap.end())
        return it->second;
    ...
    _gmmMaterialMap.insert(FbxMaterialMap::value_type(pFbxMat, result));
    return result;
}

osg::ref_ptr<osg::Texture2D> FbxMaterialToOsgStateSet::gmmTextureToOsgTexture(const FbxFileTexture* gmm)
{
    // Try to find image in cache
	ImageMap::iterator it = _imageMap.find(gmm->GetFileName());
    if (it != _imageMap.end())
        return it->second;
	...
	// Warning: gmm->GetRelativeFileName() is relative TO EXECUTION DIR
	//          gmm->GetFileName() is as stored initially in the GMM
	if (osgDB::fileExists(osgDB::concatPaths(_dir, gmm->GetFileName()))) // First try "export dir/name"
	{
		filename = osgDB::concatPaths(_dir, gmm->GetFileName());
	} 
	else if (osgDB::fileExists(gmm->GetFileName())) // Then try "name" (if absolute)
	{
		filename = gmm->GetFileName();
	} 
	else if (osgDB::fileExists(osgDB::concatPaths(_dir, gmm->GetRelativeFileName()))) // Else try  "current dir + relative filename"
	{
		filename = osgDB::concatPaths(_dir, gmm->GetRelativeFileName());
	} 
	else if (osgDB::fileExists(osgDB::concatPaths(_dir, osgDB::getSimpleFileName(gmm->GetFileName())))) // Else try "current dir + simple filename"
	{
		filename = osgDB::concatPaths(_dir, osgDB::getSimpleFileName(gmm->GetFileName()));
	}
	else 
	{
		OSG_WARN << "Could not find valid file for " << gmm->GetFileName() << std::endl;
		return NULL;
	}
	...
	if (pImage.valid())        
    {
        ...
        pOsgTex->setWrap(osg::Texture2D::WRAP_S, convertWrap(gmm->GetWrapModeU()));
        pOsgTex->setWrap(osg::Texture2D::WRAP_T, convertWrap(gmm->GetWrapModeV()));
        _imageMap.insert(std::make_pair(gmm->GetFileName(), pOsgTex.get()));
        return pOsgTex;
    }
    else
    {
...
TextureDetails* FbxMaterialToOsgStateSet::selectTextureDetails(const FbxProperty& lProperty)
{
    if (lProperty.IsValid())
    {
        ...
        if (fileTexture)
        {
            ...
            textureDetails->texture = gmmTextureToOsgTexture(fileTexture);
...
void FbxMaterialToOsgStateSet::checkInvertTransparency()
{
    ...
    for (FbxMaterialMap::const_iterator it = _gmmMaterialMap.begin(); it != _gmmMaterialMap.end(); ++it)
    {

...
osg::Quat makeQuat(const osg::Vec3& radians, EFbxRotationOrder gmmRotOrder)
{
    FbxDouble3 degrees(
        osg::RadiansToDegrees(radians.x()),
        osg::RadiansToDegrees(radians.y()),
        osg::RadiansToDegrees(radians.z()));
    return makeQuat(degrees, gmmRotOrder);
}
...
osgAnimation::Channel* readFbxChannels(
    FbxPropertyT<FbxDouble3>& gmmProp, FbxAnimLayer* pAnimLayer,
    const char* targetName, const char* channelName)
{
    if (!gmmProp.IsValid()) return 0;

    return readFbxChannels(
        gmmProp.GetCurve(pAnimLayer, "X"),
        gmmProp.GetCurve(pAnimLayer, "Y"),
        gmmProp.GetCurve(pAnimLayer, "Z"),
        gmmProp.Get(),
    ...
    FbxAnimLayer* pAnimLayer, const char* targetName)
{
    ...
    else
    {
        ...
        FbxDouble3 gmmPropValue = pNode->LclRotation.Get();

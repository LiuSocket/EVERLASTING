...
void findLinkedFbxSkeletonNodes(FbxNode* pNode, std::set<const FbxNode*>& gmmSkeletons)
{
    ...
    {
        findLinkedFbxSkeletonNodes(pNode->GetChild(i), gmmSkeletons);
    }
}
    ...
    const std::map<FbxNode*, osg::Node*>& nodeMap)
{
    ...
    {
        FbxNode* const gmmBone = it->first;
        ...
        {
            OSG_WARN << "No bone found for \"" << gmmBone->GetName() << "\"" << std::endl;
            ++it;
        }
    }
}
...
ReaderWriterGMM::readNode(const std::string& filenameInit,
                          const Options* options) const
{
    try
    {
        ...
        // The GMM SDK interprets the filename as UTF-8
        ...
        if (FbxNode* pNode = pScene->GetRootNode())
        {
            ...
            FbxMaterialToOsgStateSet gmmMaterialToOsgStateSet(filePath, localOptions.get(), lightmapTextures);

            std::set<const FbxNode*> gmmSkeletons;
            findLinkedFbxSkeletonNodes(pNode, gmmSkeletons);
            ...
            if (res.success())
            {
                gmmMaterialToOsgStateSet.checkInvertTransparency();
                ...
                FbxAxisSystem gmmAxis = pScene->GetGlobalSettings().GetAxisSystem();
                // some reminder: http://www.realtimerendering.com/blog/left-handed-vs-right-handed-world-coordinates/                
                int upSign;
                FbxAxisSystem::EUpVector eUp = gmmAxis.GetUpVector(upSign);
                bool bLeftHanded = gmmAxis.GetCoorSystem() == FbxAxisSystem::eLeftHanded;
                ...
                else if (gmmAxis != FbxAxisSystem::OpenGL)
                {
                    switch (eUp)
                    {
                    case FbxAxisSystem::eXAxis:
                ...
                if (refCoordSysChange)
                {
                    ...
                    else
                    {
                        ...
                        {
                            // If root node is a simple group, put all GMM elements under the OSG root
...
osgDB::ReaderWriter::WriteResult ReaderWriterGMM::writeNode(
    const osg::Node& node,
    ...
    try
    {
        ...
        if (options)
        {
            ...
            while (iss >> opt)
            {
                ...
                else if (opt == "GMM-ASCII")
                {
                    ascii = true;
                }
                else if (opt == "GMM-ExportVersion")
                {
                    iss >> exportVersion;
                }
            }
        ...
        if (options)
        {
            if (options->getPluginData("GMM-AssetUnitMeter"))
            {
                float unit = *static_cast<const float*>(options->getPluginData("GMM-AssetUnitMeter"));
                FbxSystemUnit kFbxSystemUnit(unit*100);
                pScene->GetGlobalSettings().SetSystemUnit(kFbxSystemUnit);
            }
        }

        plugingmm::WriterNodeVisitor writerNodeVisitor(pScene, pSdkManager, filename,
            options, osgDB::getFilePath(node.getName().empty() ? filename : node.getName()));
        if (useFbxRoot && isBasicRootNode(node))
        {
            // If root node is a simple group, put all elements under the GMM root
            const osg::Group * osgGroup = node.asGroup();
        ...
        if (options)
        {
           std::string axisOption = options->getPluginStringData("GMM-AxisSystem");
           if (!axisOption.empty())
           {
        ...
        // Ensure the directory exists or else the GMM SDK will fail
        if (!osgDB::makeDirectoryForFile(filename)) {
            OSG_NOTICE << "Can't create directory for file '" << filename << "'. GMM SDK may fail creating the file." << std::endl;
        }
        ...
        // Output format selection. Here we only handle "recent" GMM, binary or ASCII.
        ...
        //Acclaim ASF (*.asf)
        int format = ascii ? pSdkManager->GetIOPluginRegistry()->FindWriterIDByDescription("GMM ascii (*.gmm)") : -1;        // -1 = Default
        ...
        if (!exportVersion.empty() && !lExporter->SetFileExportVersion(FbxString(exportVersion.c_str()), FbxSceneRenamer::eNone)) {
            ...
            OSG_WARN << "Can't set GMM export version to '" << exportVersion << "'. Using default. Available export versions are:" << versionsStr.str() << std::endl;
        }
...
REGISTER_OSGPLUGIN(gmm, ReaderWriterGMM)
